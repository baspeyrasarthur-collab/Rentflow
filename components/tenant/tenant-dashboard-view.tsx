...
import type { getTenantDashboardData } from "@/server/tenant/dashboard";

type TenantDashboardData = Awaited<ReturnType<typeof getTenantDashboardData>>;
type DashboardStats = NonNullable<TenantDashboardData["stats"]>;
type ContractTenant = TenantDashboardData["currentRentals"][number];
type Invitation = TenantDashboardData["receivedInvitations"][number];
type Payment = TenantDashboardData["recentPayments"][number];
type DeclarablePayment =
  TenantDashboardData["externalPaymentsToDeclare"][number];
type Receipt = TenantDashboardData["recentReceipts"][number];
type RequestedReceipt = TenantDashboardData["requestedReceipts"][number];
type TenantRequest = TenantDashboardData["tenantRequests"][number];
...
type PaymentWithState = {
  ...
  latestExternalPaymentDeclaration: Payment["declarations"][number] | null;
  canDeclarePayment: boolean;
  canPayWithMock: boolean;
};

type ContractTenantWithMandateState = {
  contractTenant: ContractTenant;
  latestMandate: ContractTenant["paymentMandates"][number] | null;
  hasAcceptedMandate: boolean;
  canAcceptMockMandate: boolean;
};

export type TenantDashboardContractTenant = ContractTenant;
export type TenantDashboardDeclarablePayment = DeclarablePayment;
export type TenantDashboardPayment = Payment;
export type TenantDashboardReceipt = Receipt;
export type TenantDashboardRequestedReceipt = RequestedReceipt;
export type TenantDashboardTenantRequest = TenantRequest;
export type TenantDashboardStats = DashboardStats;

type TenantDashboardServerActions = {
  acknowledgeResolvedTenantRequest: FormAction;
  acknowledgeRefusedTenantRequest: FormAction;
  acceptInvitation: FormAction;
  declareExternalPaymentPaid: FormAction;
  declareExternalPaymentNotPaidYet: FormAction;
  requestRentReceipt: FormAction;
  markReceiptAsSeen: FormAction;
  requestContractTermination: FormAction;
  acceptMockMandate: FormAction;
  payWithMockProvider: FormAction;
};

type TenantDashboardActionRenderers = {
  tenantRequestResolved?: (tenantRequest: TenantRequest) => ReactNode;
  tenantRequestRefused?: (tenantRequest: TenantRequest) => ReactNode;
  invitationAccept?: (invitation: Invitation) => ReactNode;
  declarablePayment?: (payment: DeclarablePayment) => ReactNode;
  receiptRequest?: (payment: Payment) => ReactNode;
  availableReceipt?: (receipt: Receipt) => ReactNode;
  contractTermination?: (contractTenant: ContractTenant) => ReactNode;
  mockMandate?: (contractTenant: ContractTenant) => ReactNode;
  paymentReceiptRequest?: (payment: Payment) => ReactNode;
  paymentDeclaration?: (payment: Payment) => ReactNode;
  mockPayment?: (payment: Payment) => ReactNode;
};

type TenantDashboardViewProps = {
  stats: DashboardStats;
  receivedInvitations: Invitation[];
  contractTenantsWithMandateState: ContractTenantWithMandateState[];
  formerRentals: ContractTenant[];
  paymentsWithState: PaymentWithState[];
  requestedReceipts: RequestedReceipt[];
  recentReceipts: Receipt[];
  priorityActions: string[];
  firstDeclarablePayment: DeclarablePayment | null;
  firstReceiptRequestPayment: PaymentWithState | null;
  firstAvailableReceipt: Receipt | null;
  firstReceivedInvitation: Invitation | null;
  firstResolvedTenantRequest: TenantRequest | null;
  firstRefusedTenantRequest: TenantRequest | null;
  primaryContractRental: ContractTenant | null;
  terminationQuickRental: ContractTenant | null;
  serverActions?: TenantDashboardServerActions;
  renderActions?: TenantDashboardActionRenderers;
  notice?: ReactNode;
  contractHref?: (contractId: string) => string;
  receiptPdfHref?: (receiptId: string) => string;
  requestsHref?: string;
...
export function TenantDashboardView({
  contractHref = (contractId) => `/tenant/contracts/${contractId}`,
  contractTenantsWithMandateState,
  firstAvailableReceipt,
  firstDeclarablePayment,
  firstReceivedInvitation,
  firstReceiptRequestPayment,
  firstRefusedTenantRequest,
  firstResolvedTenantRequest,
  formerRentals,
  notice,
  paymentsWithState,
  ...
  priorityActions,
  quickActions,
  receiptPdfHref = (receiptId) => `/receipts/${receiptId}/pdf`,
  receivedInvitations,
  recentReceipts,
  renderActions,
  requestedReceipts,
  requestsHref = "/tenant/requests",
  serverActions,
  stats,
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {showTenantRequestResolvedAction && firstResolvedTenantRequest ? (
              <ActionCard
                ...
                tone="success"
              >
                {renderActions?.tenantRequestResolved ? (
                  renderActions.tenantRequestResolved(
                    firstResolvedTenantRequest,
                  )
                ) : serverActions ? (
                  <form action={serverActions.acknowledgeResolvedTenantRequest}>
                    <input
            ...
            {showTenantRequestRefusedAction && firstRefusedTenantRequest ? (
              ...
              >
                ...
                {renderActions?.tenantRequestRefused ? (
                  renderActions.tenantRequestRefused(firstRefusedTenantRequest)
                ) : serverActions ? (
                  <form action={serverActions.acknowledgeRefusedTenantRequest}>
                    <input
                      name="tenantRequestId"
                      type="hidden"
            ...
            {showInvitationAction && firstReceivedInvitation ? (
              <ActionCard
                ...
                tone="info"
              >
                {renderActions?.invitationAccept ? (
                  renderActions.invitationAccept(firstReceivedInvitation)
                ) : serverActions ? (
                  <form action={serverActions.acceptInvitation}>
                    <input
                      name="invitationId"
                      type="hidden"
              ...
              <div id="declare-rent-paid">
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {renderActions?.declarablePayment ? (
                      renderActions.declarablePayment(firstDeclarablePayment)
                    ) : serverActions ? (
                      <>
                        <form action={serverActions.declareExternalPaymentPaid}>
                          <input
                            name="paymentId"
                            type="hidden"
                        ...
                        <form
                          action={
                            serverActions.declareExternalPaymentNotPaidYet
                          }
                        >
                          <input
                            name="paymentId"
            ...
            {showReceiptRequestAction && firstReceiptRequestPayment ? (
              <ActionCard
                ...
                tone="success"
              >
                {renderActions?.receiptRequest ? (
                  renderActions.receiptRequest(
                    firstReceiptRequestPayment.payment,
                  )
                ) : serverActions ? (
                  <form action={serverActions.requestRentReceipt}>
                    <input
                      name="paymentId"
                      type="hidden"
            ...
            {showAvailableReceiptAction && firstAvailableReceipt ? (
              ...
              >
                <div className="flex flex-wrap gap-2">
                  <Link
                    className={buttonVariants({ size: "sm" })}
                    href={receiptPdfHref(firstAvailableReceipt.id)}
                  >
                    Ouvrir le PDF
                  </Link>
                  {renderActions?.availableReceipt ? (
                    renderActions.availableReceipt(firstAvailableReceipt)
                  ) : serverActions ? (
                    <form action={serverActions.markReceiptAsSeen}>
                      <input
                        name="receiptId"
                        type="hidden"
              ...
              <SpotlightCard key={invitation.id} tone="info">
                <article className="rounded-xl border border-ring/45 bg-ring/12 p-5 text-card-foreground shadow-sm shadow-black/10">
                  <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                    ...
                    <div className="min-w-0 space-y-3 lg:max-w-xs">
                      {renderActions?.invitationAccept ? (
                        renderActions.invitationAccept(invitation)
                      ) : serverActions ? (
                        <form action={serverActions.acceptInvitation}>
                          <input
                            name="invitationId"
                ...
                return (
                  ...
                  >
                    ...
                    {contractTenant.status === "ACTIVE" ? (
                      ...
                      >
                        ...
                        >
                          ...
                          {renderActions?.contractTermination ? (
                            renderActions.contractTermination(contractTenant)
                          ) : serverActions ? (
                            <form
                              action={serverActions.requestContractTermination}
                    ...
                    >
                      ...
                      {canAcceptMockMandate ? (
                        renderActions?.mockMandate ? (
                          renderActions.mockMandate(contractTenant)
                        ) : serverActions ? (
                          <form
                            action={serverActions.acceptMockMandate}
                            className="mt-3"
                          >
                            <input
                              name="contractTenantId"
                  ...
                  <article className="rounded-xl border border-border bg-card p-5 text-card-foreground shadow-sm shadow-black/10 transition-all duration-300 hover:-translate-y-0.5 hover:border-ring/55 hover:shadow-xl hover:shadow-black/20">
                    ...
                    <div className="mt-5 flex flex-wrap gap-2">
                      {receiptRequestState.canRequestReceipt ? (
                        renderActions?.paymentReceiptRequest ? (
                          renderActions.paymentReceiptRequest(payment)
                        ) : serverActions ? (
                          <form action={serverActions.requestRentReceipt}>
                            <input
                              name="paymentId"
                      ...
                      {canDeclarePayment ? (
                        renderActions?.paymentDeclaration ? (
                          renderActions.paymentDeclaration(payment)
                        ) : serverActions ? (
                          <>
                            <form
                              action={serverActions.declareExternalPaymentPaid}
                            >
                              <input
                                name="paymentId"
                                type="hidden"
                      ...
                      {canPayWithMock ? (
                        renderActions?.mockPayment ? (
                          renderActions.mockPayment(payment)
                        ) : serverActions ? (
                          <form action={serverActions.payWithMockProvider}>
                            <input
                              name="paymentId"
                              type="hidden"
                              value={payment.id}
        ...
        <div className="flex flex-wrap gap-3">
          {quickActions ?? (
            <>
              {primaryContractRental ? (
                <CompactActionPill
                  href={contractHref(primaryContractRental.rentalContract.id)}
